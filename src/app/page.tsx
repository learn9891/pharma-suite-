...
export default async function DashboardPage() {
  ...
  const lowStock = medicines.filter(
    (medicine) => medicine.stockQty <= medicine.reorderLevel,
  );
  const stockValue = medicines.reduce(
    (sum, medicine) => sum + medicine.unitPrice * medicine.stockQty,
    0,
  );
  ...
  return (
    <div className="space-y-8">
      <section className="relative overflow-hidden rounded-2xl border border-white/60 bg-gradient-to-br from-teal-600 via-teal-500 to-sky-600 p-8 text-white shadow-lg">
        ...
        <div className="relative flex flex-wrap items-center justify-between gap-6 animate-fade-up">
          <div className="min-w-0 flex-1">
            <p className="text-xs uppercase tracking-[0.2em] text-white/75">
              Pharmacy control room
            </p>
            <h1 className="mt-2 text-3xl font-semibold tracking-tight sm:text-4xl">
              Everything on the counter, in one view
            </h1>
            <p className="mt-3 max-w-2xl text-sm text-white/85">
              ...
              . Track stock, analyse prescriptions, ask the assistant and bill
              with GST — without leaving the app.
            </p>
              ...
              {lowStock.slice(0, 6).map((medicine, index) => (
                ...
                >
                  ...
                  <span className="font-medium text-rose-600">
                    {medicine.stockQty} left / reorder at{" "}
        ...
        <Reveal delay={120}>
          <section className="card p-5">
            ...
            <ul className="mt-3 divide-y divide-slate-100 text-sm">
              ...
              {expiring.length === 0 && (
                <li className="py-2 text-slate-500">
                  No batch expires in the next 90 days.
                </li>
              )}
            </ul>
          </section>
